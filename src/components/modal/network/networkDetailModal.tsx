  ...
  return (
    ...
    >
      ...
      <div className="p-5 font-pretendard border-b border-grey_1 bg-white">
        <h2 className="text-lg font-bold">{`Network Details - ${
          data?.Name || 'Unknown Network'
        }`}</h2>
      ...
      <DialogContent className="overflow-y-auto flex-grow bg-grey_0">
        ...
        <div className="my-5">
          ...
          {Object.keys(data?.Containers || {}).length > 0 ? (
                  ...
                  {Object.values(data.Containers).map(
                    (container: any, index: number) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>{container.IPv4Address || 'N/A'}</TableCell>
            ...
            <p className="text-sm text-grey_4 mt-2">No containers available</p>
          )}
        </div>
